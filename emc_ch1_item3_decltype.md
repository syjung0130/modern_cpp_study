...
그래서 C++14를 사용할 경우, 함수를 아래와 같이 정의하는 것이 가능하다.
~~~C++
template<typename Container, typename Index>  // C++14
auto authAndAccess(Container& c, Index i)     // 아주 정확한 것은 아님.
{
    authenticateUser();
...
컨테이너 c는 non-const 객체에 대한 왼값 참조로서 함수에 전달되고 있는데,(함수의 return값을 클라이언트가 수정하기 위함)  
문제는 이 때문에 함수에 오른값 컨테이너는 전달할 수 없다는 것이다.  
(오른 값을 왼값 참조에 묶을 수 없기 때문이다. const에 대한 왼값 참조에는 묶을 수 있지만.. )  
...
중복적재를 사용할 수도 있지만(왼값 참조 매개변수를 받는 중복적재 버전과  
오른값 참조 매개변수를 받는 중복적재 버전을 따로 만들어서),  
그러면 관리해야 할 함수가 두개가 된다.  
...
x는 변수의 이름이므로 decltype(x)는 int이다.  
